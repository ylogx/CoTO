...
def rest(driver: WebDriver, doer: Doer, info: HealthInfo):
    ...
    loc_name = doer.ele_css('div.tw-border:nth-child(1) > div:nth-child(1) > h2:nth-child(1)').text
    logger.info(f"First location is {loc_name}")

    # Click button: div.tw-border:nth-child(1) > div:nth-child(2) > button:nth-child(1)
    availability_btn = doer.ele_css('div.tw-border:nth-child(1) > div:nth-child(2) > button:nth-child(1)')
...
def fill_hc_info(driver: WebDriver, doer: Doer, info: HealthInfo):
    doer.wait_till_url('https://covid19.ontariohealth.ca/app-identity?viewId=U5GBUDFZ526Z')
